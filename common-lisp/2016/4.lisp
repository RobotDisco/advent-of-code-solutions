...
(defun parse-room (str)
  (let* ((parts (cl-utilities:split-sequence #\- str))
	 (chars (format nil "~{~a~}" (butlast parts)))
	 (chars1 (format nil "~{~a~^-~}" (butlast parts)))
	 (checksum (car (last parts)))
	 (checksum1 (cl-ppcre:register-groups-bind (first second) ("(\\d+)\\[(\[a-z\]+)\\]" checksum) (list (parse-integer first) second)))
	 (hash (make-hash-table)))
    (loop for char across chars
       do (incf (gethash char hash 0)))
    (let ((gen-checksum (coerce (sort (sort (loop for k being the hash-key of hash collect k) #'char-lessp)
				      (lambda (k1 k2)
					(> (gethash k1 hash)
					    (gethash k2 hash)))) 'string)))
      (list checksum1 (subseq gen-checksum 0 5) chars1))))



(defun q4 (str)
  (let* ((rooms (mapcar #'parse-room (cl-utilities:split-sequence #\newline str)))
	 (rooms1 (reduce (lambda (accum x)
			    (if (string= (cadar x) (cadr x))
				(+ accum (caar x))
				accum)) rooms :initial-value 0)))
    rooms1))

(defun f (idx str)
  (let* ((strs (cl-utilities:split-sequence #\- str))
	 (chars (mapcar (lambda (x) (coerce x 'list)) strs))
	 (ints (loop for x in chars collect (mapcar #'char-code x)))
	 (ints0 (loop for x in ints collect (mapcar (lambda (x) (+ (mod (+ (- x 97) idx) 26) 97)) x)))
	 (chars0 (loop for x in ints0 collect (mapcar #'code-char x)))
	 (strs0 (mapcar (lambda (x) (coerce x 'string)) chars0)))
    (cons idx (format nil "~{~A~^ ~}" strs0))))

(defun q42 (str)
  (let* ((rooms (mapcar #'parse-room (cl-utilities:split-sequence #\newline str)))
	 (rooms1 (remove-if-not (lambda (x)
				  (string= (cadar x) (cadr x))) rooms))
	 (rooms2 (mapcar (lambda (x) (list (caar x) (caddr x))) rooms1))
	 (deciphered (mapcar (lambda (x) (f (car x) (cadr x))) rooms2)))
    (find-if-not (lambda (x) (null (cl-ppcre:scan "north" (cdr x)))) deciphered)))
