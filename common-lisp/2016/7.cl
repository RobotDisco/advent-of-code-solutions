...
(defun abbap (str)
  (and (= (length str) 4)
       (string/= (subseq str 0 1) (subseq str 1 2))
...
(defun aba-babp (aba bab)
  (and (abap aba)
       (abap bab)
       (string= (subseq aba 0 1) (subseq bab 1 2))
       (string= (subseq bab 0 1) (subseq aba 1 2))))
...
(defun split-string (str)
  (let ((abas '())
	...
	(hnet nil)
	(candidate '()))
    (dolist (c (coerce str 'list) (progn
				    (when candidate (setq abas (cons (reverse (coerce candidate 'string)) abas)))
				    (values abas babs)))
      (when (char= c #\[)
	(setq hnet t)
	(setq abas (cons (reverse (coerce candidate 'string)) abas))
	(setq candidate '()))
      (when (char= c #\])
	(setq hnet nil)
	(setq babs (cons (reverse (coerce candidate 'string)) babs))
	(setq candidate '()))
      (unless (or (char= c #\]) (char= c #\[))
	(push c candidate)))))

(defun tlsp (str)
  (let* ((wlength (length str))
	 (ipp nil)
	 (disqualified nil))
    (do ((candidate "")
	 (i 0)
	 (hnet nil))
	((> i (- wlength 4)))
      (setq candidate (subseq str i (+ i 4)))
      (when (char= (elt candidate 0) #\[) (setq hnet t))
      (when (char= (elt candidate 3) #\]) (setq hnet nil))
      (when (abbap candidate)
	(if hnet
	    (setq disqualified t)
	    (setq ipp t)))
      (incf i))
    (and (not disqualified) ipp)))

(defun sslp (str)
  (multiple-value-bind (abas babs) (split-string str)
    (some (lambda (aba)
	    (let ((wlength (length aba))
		  (ssls nil))
	      (do ((candidate "")
		   (i 0))
		  ((> i (- wlength 3)))
		(setq candidate (subseq aba i (+ i 3)))
		(setq ssls
		      (or ssls
			  (some #'identity
				(mapcar (lambda (bab)
					  (let ((wlength (length bab))
						(ssls nil))
					    (do ((candidate1 "")
						 (i 0))
						((> i (- wlength 3)))
					      (setq candidate1 (subseq bab i (+ i 3)))
					      (when (aba-babp candidate candidate1) (setq ssls t))
					      (incf i))
					    ssls))
					babs))))
		(incf i))
...
(defun q7 (input)
  (let ((strs (cl-utilities:split-sequence #\newline input)))
    (count t (mapcar #'tlsp strs))))

(defun q7_2 (input)
  (let ((strs (cl-utilities:split-sequence #\newline input)))
    (count t (mapcar #'sslp strs))))
